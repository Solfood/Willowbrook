...
import { createInitialState, planReducer, actions, MAX_HISTORY } from '../src/features/plan/planReducer.js';
...
test('ADD_BED appends a bed and grows history', () => {
    const state = planReducer(freshState(), actions.addBed({
...
test('UPDATE_BED patches fields', () => {
    let s = planReducer(freshState(), actions.addBed({ id: 'b1', name: 'A', widthFt: 4, lengthFt: 4 }));
    s = planReducer(s, actions.updateBed('b1', { name: 'A2', widthFt: 4, lengthFt: 8 }));
...
test('REMOVE_BED removes bed and its plantings + journal', () => {
    let s = planReducer(freshState(), actions.addBed({ id: 'b1', name: 'A', widthFt: 4, lengthFt: 4 }));
    s = planReducer(s, actions.addPlanting({ id: 'p1', bedId: 'b1', plantId: 'tomato', quantity: 3, status: 'planned', datePlanted: null, notes: '' }));
    s = planReducer(s, actions.addJournalEntry({ id: 'j1', bedId: 'b1', date: '2026-05-01', text: 'note' }));
    s = planReducer(s, actions.removeBed('b1'));
...
test('ADD_PLANTING / UPDATE_PLANTING / REMOVE_PLANTING', () => {
    ...
    s = planReducer(s, actions.updatePlanting('p1', { quantity: 5 }));
    assert.equal(s.plan.plantings[0].quantity, 5);
    s = planReducer(s, actions.removePlanting('p1'));
    assert.equal(s.plan.plantings.length, 0);
});
...
test('MARK_TASK_DONE advances planting status', () => {
    let s = planReducer(freshState(), actions.addBed({ id: 'b1', name: 'A', widthFt: 4, lengthFt: 4 }));
    s = planReducer(s, actions.addPlanting({ id: 'p1', bedId: 'b1', plantId: 'tomato', quantity: 3, status: 'planned', datePlanted: null, notes: '' }));
    s = planReducer(s, actions.markTaskDone({ plantingId: 'p1', nextStatus: 'sown_indoors', date: '2026-03-01' }));
...
test('ADD_JOURNAL_ENTRY', () => {
    let s = planReducer(freshState(), actions.addBed({ id: 'b1', name: 'A', widthFt: 4, lengthFt: 4 }));
    s = planReducer(s, actions.addJournalEntry({ id: 'j1', bedId: 'b1', date: '2026-05-01', text: 'tomatoes sprouted' }));
    assert.equal(s.plan.journal.length, 1);
});

test('ADD_CUSTOM_PLANT / UPDATE_CUSTOM_PLANT', () => {
    let s = planReducer(freshState(), actions.addCustomPlant({
    ...
    s = planReducer(s, actions.updateCustomPlant('custom-okra', { spacingInches: 18 }));
    assert.equal(s.plan.customPlants[0].spacingInches, 18);
});

test('UNDO / REDO walk the history', () => {
    let s = planReducer(freshState(), actions.addBed({ id: 'b1', name: 'A', widthFt: 4, lengthFt: 4 }));
    s = planReducer(s, actions.addBed({ id: 'b2', name: 'B', widthFt: 4, lengthFt: 4 }));
    assert.equal(s.plan.beds.length, 2);
    s = planReducer(s, actions.undo());
    assert.equal(s.plan.beds.length, 1);
    s = planReducer(s, actions.redo());
    assert.equal(s.plan.beds.length, 2);
});
...
test('LOAD_PLAN replaces state and grows history', () => {
    ...
    s = planReducer(s, actions.loadPlan(fresh));
...
test('history is capped at MAX_HISTORY', () => {
    ...
    for (let i = 0; i < MAX_HISTORY + 10; i++) {
        s = planReducer(s, actions.addBed({ id: `b${i}`, name: `B${i}`, widthFt: 4, lengthFt: 4 }));
    }
